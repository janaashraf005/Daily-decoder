            ...
            for image_path, ref_keypoints, ref_descriptors in reference_data:
                ...
                if ref_descriptors is not None:
                    ...
                    if similarity > similarity_threshold:
                        ...
                        cv2.putText(frame, f"{image_path}: {similarity:.2f}",
                                    (10, 30 + 30 * reference_data.index((image_path, ref_keypoints, ref_descriptors))),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

                        # Trigger the popup if specific images are detected
                        if image_path == "Rubbish_Sorting_System_Germany.jpg":
                            text = """Rubbish Sorting in Germany

                            Yellow Bin: Plastic, metal, and Tetra Paks (clean before disposing).
                            Blue Bin: Paper and cardboard (flatten boxes to save space).
                            Brown Bin: Organic waste (no plastic bags).
                            Black Bin: General waste.
                            Glass: Use separate bins for clear, green, and brown glass."""

                            show_popup(text)


                        elif image_path == "Public_Laundry_Machines_France_Laundromats.jpg":
                            text = """ Public Laundry Machines in France

Preparation: Bring detergent or buy single-use packs at the laundromat.
Washing: Load clothes, add detergent, and select the wash cycle based on fabric or temperature.
Payment: Insert coins or pay by card as instructed on the machine.
Drying: Move clothes to a dryer, pay, and select time and temperature."""

                            show_popup(text)
                        elif image_path == "Tokyo_Train_Ticket_Machine.jpg":
                            text = """ Tokyo Train Ticket Machines

                            Finding Your Fare: Check the metro map above the machine for your destination and fare (in yen).
Buying a Ticket: Select English (if available), press the fare amount, insert coins/bills, and collect your ticket and change.
Using Suica/Pasmo Cards: Buy or recharge at the machine, select "Charge," add funds, and tap at ticket gates to enter/exit.
